...
首先请观察您的安装器的设置页面。安装器默认从```Onedrive For Business```下载。  
但是，若您无法从该渠道下载，您目前有多种选择：  
- 首先，我们推荐您尝试从```拉斯维加斯服务器with CDN```下载。  
请选择该项，然后至“安装页面”重新下载，下载完毕后查看您的游戏根目录文件是否符合上图。
- 其次，我们推荐您尝试```从GitHub下载文件```。  
...
以下记述的常见问题均为MelonLoader旧版的常见问题。MelonLoader版本即将停止支持。请使用**最新版**安装器下载BepInEx版本。
